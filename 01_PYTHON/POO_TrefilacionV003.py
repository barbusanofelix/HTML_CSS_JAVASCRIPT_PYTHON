...
class Trefilado:
    ...
    def calcular_serie(self):
        ...
        # Pasos restantes (disminución lineal del diámetro)
        pasos_restantes = self.num_pasos - 3
        decremento_diametro = (self.diametros[-1] - self.diametro_final) / pasos_restantes

        for _ in range(pasos_restantes):
            diametro_actual = self.diametros[-1] - decremento_diametro
            self.diametros.append(diametro_actual)

        # Cálculo de reducciones de área
        for i in range(1, self.num_pasos):
            area_anterior = math.pi * (self.diametros[i - 1] / 2) ** 2
            area_actual = math.pi * (self.diametros[i] / 2) ** 2
            reduccion_area = (area_anterior - area_actual) / area_anterior * 100
            self.reducciones_area.append(reduccion_area)

        # Insertar las reducciones iniciales
        self.reducciones_area.insert(0, self.reduccion_inicial_1 * 100)
        self.reducciones_area.insert(1, self.reduccion_inicial_2 * 100)
    ...
    def cociente_ei(self, pases):
        serie_arimetcia= sum(range(1,pases))
        return serie_arimetcia
...
print(trefilado.cociente_ei(num_pasos))