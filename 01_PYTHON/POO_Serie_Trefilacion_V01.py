...
class Trefilado:
    ...
    def calcular_serie(self):
        ...
        # Pasos restantes (reducción logarítmica uniforme)
        area_final = math.pi * (self.diametro_final / 2) ** 2
        deformacion_total = math.log(area_actual / area_final)
        pasos_restantes = self.num_pasos - 2
        deformacion_paso = deformacion_total / pasos_restantes

        for _ in range(pasos_restantes):
            area_anterior = area_actual
            area_actual *= math.exp(-deformacion_paso)
            diametro_actual = 2 * math.sqrt(area_actual / math.pi)
            self.diametros.append(diametro_actual)
